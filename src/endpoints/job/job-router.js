...
jobRouter.route('/all/:company/:time').get(async (req, res) => {
	...
	jobService.getJobs(db, company, timeBetween.now, timeBetween.date).then(allJobsForCompany => {
...
jobRouter.route('/allJobs/:time').get(async (req, res) => {
	...
	jobService.getAllJobs(db, timeBetween.now, timeBetween.date).then(allJobsWithinTimeframe => {
		res.send({
