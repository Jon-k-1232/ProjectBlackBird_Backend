...
transactionsRouter.route('/all/:time').get(async (req, res) => {
	...
	transactionService.getTransactions(db, timeBetween.now, timeBetween.date).then(allTransactions => {
		res.send({
			allTransactions,
			status: 200,
		})
...
transactionsRouter.route('/companyTransactions/:company/:time').get(async (req, res) => {
	...
	transactionService.getCompanyTransactions(db, company, timeBetween.now, timeBetween.date).then(sortedCompanyTransactions => {
		res.send({
